...
class DetectionBloc extends Bloc<DetectionEvent, DetectionState> {
  ...
  DetectionBloc({
    ...
    on<AnalyzeFreshnessOnly>(_onAnalyzeFreshnessOnly);
  }
  ...
  Future<void> _onPickImageFromCamera(
    ...
    try {
      ...
      if (imageFile != null) {
        if (event.isFreshnessOnly) {
          add(AnalyzeFreshnessOnly(imageFile));
        } else {
  ...
  Future<void> _onAnalyzeFreshnessOnly(
    AnalyzeFreshnessOnly event,
    Emitter<DetectionState> emit,
  ) async {
    emit(const DetectionLoading('Analyzing freshness...'));

    try {
      final result = await freshnessService.predict(event.imageFile);
      if (result != null) {
        emit(FreshnessAnalysisSuccess(
          imageFile: event.imageFile,
          result: result,
        ));
      } else {
        emit(const DetectionError(
            'Freshness analysis failed: No result returned'));
      }
    } catch (e) {
      emit(DetectionError('Freshness analysis failed: ${_getErrorMessage(e)}'));
    }
  }
